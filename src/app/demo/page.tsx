...
async function DatabaseInfo() {
  ...
  try {
    ...
    const result = await sql`SELECT * FROM ${sql('demo')} ORDER BY created_at DESC LIMIT 10`;
    ...
    <div className='mt-8 space-y-4'>
      ...
      <div className='bg-secondary overflow-x-auto rounded-lg p-4 font-mono text-sm text-green-400'>
        <div className='text-muted-foreground mb-2'>
          SELECT * FROM demo ORDER BY created_at DESC LIMIT 10;
        </div>
        {error ? (
          <div className='text-danger'>Error: {error}</div>
        ) : tableData.length === 0 ? (
          <div className='text-warning'>No records found in demo table</div>
        ) : (
      ...
      <div className='text-sm text-gray-600'>
        ...
        <code className='bg-primary rounded px-1'>demo</code> table
      </div>
    </div>
  );
}
